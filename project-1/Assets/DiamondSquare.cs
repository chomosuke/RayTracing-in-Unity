...
public class DiamondSquare : MonoBehaviour
{
    ...
    private Vector3[] GenerateVectors(int iterations){
        ...
        Vector3[] vectors = new Vector3[grid.Length^2];
    ...
    private float[][] GenerateHeightMap(int iterations){
        ...
        for (int h = 0; h < iterations; h++)
        {
            ...
            {
                for (int j = 0; j < length; j+=currentLength)
                {
                    grid[i+(currentLength/2)][j+(currentLength/2)]
                        = (grid[i][j] + grid[i+currentLength][j] 
                        + grid[i][j+currentLength] + grid[i+currentLength][j+currentLength]) / 4 
                        + RandomRange(randomness/2, -randomness/2);
                }
            }

            // Square Step
            for (int i = currentLength/2; i < length; i+=currentLength)
            {
                for (int j = currentLength/2; j < length; j+=currentLength)
                {
    ...
    private int[] GenerateTriangles(Vector3[] vectorsPreTriangle){
        ...
        int[] triangles = new int[(squaresOnEdge^2)*6];
        int x = 0;
        int y = 0;
        for (int i = 0; i < triangles.Length; i+=6)
        {
            triangles[i] = y*(squaresOnEdge+1)+x;
            triangles[i+1] = y*(squaresOnEdge+1)+x+1;
            triangles[i+2] = (y+1)*(squaresOnEdge+1)+x;
