		...
		Pass
		{
			...
			/* + _Time.y*/
				...
				for (i = 0; i < seedsSize * seedsSize; i++) {
					delta += sin(distance(seeds[i], v.xz) * 250 + _Time.y) / 125;
					delta += sin(distance(seeds[i], v.xz) * 50 + _Time.y) / 25;
					delta += sin(distance(seeds[i], v.xz) * 10 + _Time.y) / 5;
					delta += sin(distance(seeds[i], v.xz) * 2 + _Time.y);
				}
				return offset + delta * waveHeight;
			}
